...
class DinoWrapper(torch.nn.Module):
    def __init__(self):
        ...
        # self.model = torch.hub.load(
        #     "/yuxi/project/latent-diffusion/configs/latent-diffusion/dinov2",  # 本地路径，注意替换成你自己的真实路径
        #     "dinov2_vitb14",
        #     source="local",
        #     trust_repo=True
        # )
...
class DinoLatentDiffusion(DDPM):
    ...
    def __init__(
        ...
        cond_stage_key1="segmentation",
        cond_stage_key2="reference",
    ...
    def get_input(
        ...
        if self.model.conditioning_key is not None:
            if cond_key1 is None:
                cond_key1 = self.cond_stage_key1
            if cond_key2 is None:
                cond_key2 = self.cond_stage_key2
            if cond_key1 != self.first_stage_key:
                if cond_key1 in ["caption", "coordinates_bbox"]:
                    xc1 = batch[cond_key1]
                elif cond_key1 == "class_label":
                    xc1 = batch
                else:
                    xc1 = super().get_input(batch, cond_key1).to(self.device)
            else:
                xc1 = x
            if cond_key2 != self.first_stage_key:
                if cond_key2 in ["caption", "coordinates_bbox"]:
                    xc2 = batch[cond_key2]
                elif cond_key2 == "class_label":
                    xc2 = batch
                else:
                    xc2 = super().get_input(batch, cond_key2).to(self.device)
            else:
                xc2 = x
            if not self.cond_stage_trainable or force_c_encode:
                if isinstance(xc1, dict) or isinstance(xc1, list):
                    # import pudb; pudb.set_trace()
                    c1 = self.cond_stage_model.encode(xc1)
                else:
                    c1 = self.cond_stage_model.encode(xc1.to(self.device))
                if isinstance(xc2, dict) or isinstance(xc2, list):
                    # import pudb; pudb.set_trace()
                    c2 = self.cond_stage_model.encode(xc2)
                else:
                    c2 = self.cond_stage_model.encode(xc2.to(self.device))
            else:
                c1 = xc1
                c2 = xc2
            if bs is not None:
    ...
    def log_images(
        ...
        **kwargs,
        ...
        log = dict()
        z, c1, c2, x, xrec, xc1, xc2 = self.get_input(
            batch,
...
class Layout2ImgDiffusion(DinoLatentDiffusion):
