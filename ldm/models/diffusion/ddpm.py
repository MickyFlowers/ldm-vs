        ...
        if self.model.conditioning_key is not None:
            if hasattr(self.cond_stage_model, "decode"):
                ...
                if xc.shape[1] > 3:
                    xc_ref = xc[:, :3, ...]
                    xc_peg = xc[:, 3:, ...]
                    log["conditioning_peg"] = xc_peg
                    log["conditioning_ref"] = xc_ref
                else:
                    log["conditioning"] = xc
        ...
        if self.model.conditioning_key is not None:
            ...
            if not self.cond_stage_trainable or force_c_encode:
                if isinstance(xc1, dict) or isinstance(xc1, list):
                    # import pudb; pudb.set_trace()
                    c1 = self.cond_stage_model.encode(xc1).mode()
                else:
                    c1 = self.cond_stage_model.encode(xc1.to(self.device)).mode()
                if isinstance(xc2, dict) or isinstance(xc2, list):
                    # import pudb; pudb.set_trace()
                    c2 = self.first_stage_model.encode(xc2).mode()
                else:
                    c2 = self.first_stage_model.encode(xc2.to(self.device)).mode()
            else:
                c1 = xc1
                c2 = xc2
            if bs is not None:
