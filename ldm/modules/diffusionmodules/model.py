        ...
        **ignore_kwargs,
        ...
        for i_level in range(self.num_resolutions):
            ...
            for i_block in range(self.num_res_blocks):
                ...
                block_in = block_out
                if curr_res in attn_resolutions:
                    print(f"Encoder: Resolution {curr_res} add attention block")
                    attn.append(make_attn(block_in, attn_type=attn_type))
            down = nn.Module()
            down.block = block
            down.attn = attn
            if i_level != self.num_resolutions - 1:
                down.downsample = Downsample(block_in, resamp_with_conv)
                curr_res = list(map(lambda x: x // 2, curr_res))
            self.down.append(down)
...
class Decoder(nn.Module):
    def __init__(
        ...
        **ignorekwargs,
        ...
        curr_res = list(map(lambda x: x // 2 ** (self.num_resolutions - 1), resolution))
        ...
        for i_level in reversed(range(self.num_resolutions)):
            ...
            for i_block in range(self.num_res_blocks + 1):
                ...
                block_in = block_out
                if curr_res in attn_resolutions:
                    print(f"Decoder: Resolution {curr_res} add attention block")
                    attn.append(make_attn(block_in, attn_type=attn_type))
            up = nn.Module()
            up.block = block
            up.attn = attn
