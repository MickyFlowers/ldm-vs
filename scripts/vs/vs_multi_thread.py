...
class VsMutliThread:
    def __init__(self, args):
        ...
        if not opt.image_only:
            ...
            # if gripper_enable:
            #     gripper.activate()
        if args.controller == "IBVS":
            self.controller = IBVS(
                self.camera, kp_algo=KpMatchAlgo, match_threshold=0.4
            )
        elif args.controller == "ROMA":
            self.controller = IBVS(self.camera, kp_algo=RomaMatchAlgo)
        elif args.controller == "CNSv2":
            self.controller = CNSv2(self.camera)
        ...
        input_reference = cv2.resize(
            input_reference, (0, 0), fx=self.args.scale, fy=self.args.scale
        )
        self.input_reference = transform(input_reference, device=self.device)
    ...
    def init(self):
        if not opt.image_only:
            ...
            cv2.imwrite("temp/input.jpg", color_image)
            # color_image = cv2.resize(
            #     color_image, (0, 0), fx=self.args.scale, fy=self.args.scale
            # )
            color_image = cv2.imread("temp/input.jpg")
            ...
            conditioning = cv2.resize(
                conditioning, (0, 0), fx=self.args.scale, fy=self.args.scale
            )
            conditioning = transform(conditioning, device=self.device)
            with torch.no_grad():
                with self.model.ema_scope():
                    c1 = self.model.cond_stage_model.encode(conditioning)
                    c2 = self.model.cond_stage_model.encode(self.input_reference)
                    ...
                    if self.args.controller == "CNSv2":
                        reference_image = cv2.resize(
                            reference_image,
                            (0, 0),
                            fx=1.0 / self.args.scale,
                            fy=1.0 / self.args.scale,
                        )
                    self.reference_image = reference_image.astype(np.uint8)
                    cv2.imwrite("temp/reference.jpg", self.reference_image)
    ...
    def control_thread_func(self):
        ...
        while True:
            if self.enable_vs:
                color_image, depth_image = self.camera.get_frame()
                cv2.imwrite(f"temp/video/first_image/{num_frames:03d}.jpg", color_image)
                ...
                reference_depth[:] = 0.4
                with self.lock:
                    if self.args.controller == "ROMA" or self.args.controller == "IBVS":
                        color_image = cv2.resize(
                            color_image,
                            (0, 0),
                            fx=self.args.scale,
                        ...
                        _, vel, score, match_img = self.controller.calc_vel(
                            mask=self.mask
                        )
                    elif self.args.controller == "CNSv2":
                        self.controller.update(
                            cur_img=color_image,
                            tar_img=self.reference_image,
                        )
                        _, vel, score, match_img = self.controller.calc_vel(
                            depth_hint=0.15, mask=self.mask
                        )
                if vel is None:
                    continue
                ...
                if self.args.controller == "ROMA" or self.args.controller == "IBVS":
                    ...
                    #     vel = vel / np.linalg.norm(vel) * 0.3
                    self.ur_robot.applyTcpVel(vel * 0.2)
                elif self.args.controller == "CNSv2":
                    if np.linalg.norm(vel) > 0.05:
                        vel = vel / np.linalg.norm(vel) * 0.05
                    self.ur_robot.applyTcpVel(vel * 0.8)
                num_frames += 1
                cv2.imshow("match_img", match_img)
                cv2.imwrite(f"temp/video/match_image/{num_frames:03d}.jpg", match_img)
...
parser.add_argument(
    "--tcp_pose_file",
    type=str,
    default="data/vs_examples/extrinsic/vs_tcp_pose.npy",
    help="tcp pose file",
)
parser.add_argument(
    "--camera2tcp_file",
