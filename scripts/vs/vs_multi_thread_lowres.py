        ...
        while True:
            if self.enable_vs:
                ...
                depth_image = cv2.resize(
                    depth_image, (0, 0), fx=self.args.scale, fy=self.args.scale
                )
                ...
                with self.lock:
                    if self.args.use_roma:
                        ...
                        _, vel, score, match_img = self.argsibvs_controller.calc_vel(
                            mask=self.mask
                        )
                logging.info(f"vel norm: {np.linalg.norm(vel)}, {vel=}")
                if np.linalg.norm(vel) < self.args.threshold:
                    num_threshold += 1
                    if num_threshold >= 15:
                        now_time = time.time()
    ...
    def seg_thread_func(self):
        while True:
            ...
            if self.enable_vs and self.first_mask:
                with self.lock:
                    color_image = cv2.resize(
                        self.color_image, (0, 0), fx=self.args.scale, fy=self.args.scale
                    )
...
parser.add_argument(
    "--config",
    type=str,
    default="logs/2025-08-29T20-26-10_charge-m-low-res/configs/2025-08-29T20-26-10-project.yaml",
    help="config file",
)
parser.add_argument(
    "--input_reference_path",
    type=str,
    default="/mnt/workspace/cyxovo/dataset/2025-08-28-charge-multi-finetune/img/000-012.jpg",
    help="input image",
)
parser.add_argument(
    "--ckpt_path",
    type=str,
    default="logs/2025-08-29T20-26-10_charge-m-low-res/checkpoints/epoch=000618.ckpt",
    help="ckpt path",
)
parser.add_argument(
    "--tcp_pose_file",
