...
left_arm_pose[:3, 3] =np.array([-9.53320515e-02, 5.89403488e-01, 3.64874836e-01])
left_arm_pose[:3, :3] =[[-0.09741014 ,-0.80472018  ,0.58560797],
                        [-0.99417627 , 0.05142537 ,-0.09470467],
                        [ 0.04609566 ,-0.59142274 ,-0.80504306]]
...
# left_arm_error_pos_lower = np.array([-0.005, -0.005, -0.01])
...
left_arm_error_pos_lower = np.array([-0.03, -0.03, -0.03])
left_arm_error_pos_upper = np.array([0.03, 0.03, 0.0])
...
for i in range(900,1200):
    ...
    while count < 10:
        ...
        # 确保保存目录存在
        save_dir_image = "/mnt/workspace/cyxovo/dataset/charge_data_random/img"
   
        os.makedirs(save_dir_image, exist_ok=True)
        # 构造完整文件路径
        filename_img = f"{save_dir_image}/{i:05d}_{count:05d}.jpg"
