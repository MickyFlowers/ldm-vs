...
# root_path = "/mnt/workspace/cyxovo/dataset/charge_data_random/img"
# new_root_path = "/mnt/workspace/cyxovo/dataset/charge_data_random/rename_img"
# os.makedirs(new_root_path, exist_ok=True)
# files = os.listdir(root_path)
# files.sort(
#     key=lambda x: int(os.path.splitext(x)[0].split("_")[0]) * 10
#     + int(os.path.splitext(x)[0].split("_")[1])
# )

# for i, file in enumerate(files):
#     count = int(os.path.splitext(file)[0].split("_")[0]) * 10 + int(
...
# rename seg images
root_path = "/mnt/workspace/cyxovo/dataset/charge_data_random/rename_seg"
new_root_path = "/mnt/workspace/cyxovo/dataset/charge_data_random/seg"
os.makedirs(new_root_path, exist_ok=True)
files = os.listdir(root_path)
files.sort(key=lambda x: int(os.path.splitext(x)[0]))
for i, file in enumerate(files):
    count1 = int(os.path.splitext(file)[0]) // 10
    count2 = int(os.path.splitext(file)[0]) % 10
    img = cv2.imread(os.path.join(root_path, file))
    cv2.imwrite(os.path.join(new_root_path, f"{count1:05d}-{count2:05d}.jpg"), img)
