...
# rename img images
# root_path = "../data/good_random_data_single_2/img"
# new_root_path = "../data/good_random_data_single_2/rename_img"
# files = os.listdir(root_path)
# files.sort(
#     key=lambda x: int(os.path.splitext(x)[0].split("-")[0]) * 2
#     + int(os.path.splitext(x)[0].split("-")[1])
# )
# for i, file in enumerate(files):
#     count = int(os.path.splitext(file)[0].split("-")[0]) * 2 + int(
...
#rename seg images
root_path = "../data/good_random_data_single_2/rename_seg"
new_root_path = "../data/good_random_data_single_2/seg"
files = os.listdir(root_path)
files.sort(key=lambda x: int(os.path.splitext(x)[0]))
for i, file in enumerate(files):
    count1 = int(os.path.splitext(file)[0]) // 2
    count2 = int(os.path.splitext(file)[0]) % 2
    img = cv2.imread(os.path.join(root_path, file))
